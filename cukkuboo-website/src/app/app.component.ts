  ...
  this.userService.getProfile().subscribe({
    next: (response) => {
      if (response.success) {
        ...
        const notificationCount = response.data?.notifications ?? 0;
        console.log('Notification Count:', notificationCount);

        // store unread notification count for header
        this.storageService.updateItem('unread_notifications', notificationCount);
      }
